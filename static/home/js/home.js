...
let unidadesSaude = [];
let abasCharts = new Map(); // Armazenar gráficos por aba
let abaAtiva = 'geral';
...
async function criarGraficoComparacao(filtro, estilo, unidadeSaude = null) {
    ...
    // Obter informações do indicador para título
    let indicadorInfo = getIndicadorData(filtro, unidadeSaude);
    
    // Se não tiver dados e precisar buscar por unidade
    if (!indicadorInfo && unidadeSaude !== null) {
        indicadorInfo = await buscarIndicadorPorUnidade(filtro, unidadeSaude);
    }
    ...
    if (estilo === 'line') {
        criarGraficoTemporal(filtro, chartId, containerId, container, unidadeSaude);
...
async function adicionarGraficoComparacao() {
    ...
    const unidadeSaude = document.getElementById('comparacao-unidade').value || null;
    
    if (!indicadoresData && !unidadeSaude) {
        alert('Aguarde o carregamento dos dados antes de adicionar gráficos.');
        return;
    }
    
    const resultado = await criarGraficoComparacao(filtro, estilo, unidadeSaude);
    if (resultado) {
...
function criarAbaUnidade(unidade) {
    ...
    // Criar botão da aba
    const unidadeId = sanitizarId(unidade);
...
function trocarAba(unidade) {
    ...
    const tabId = unidade === 'geral' ? 'tab-geral' : `tab-${sanitizarId(unidade)}`;
    ...
    const panelId = unidade === 'geral' ? 'panel-geral' : `panel-${sanitizarId(unidade)}`;
...
function carregarDadosAba(unidade) {
    const panelId = `panel-${sanitizarId(unidade)}`;
...
function criarGraficosAba(panelId, data) {
    ...
    document.getElementById(`status1-${panelId}`).textContent = getStatusIndicator(perc1);

    const perc2 = calculatePositivePercentage(
        data.consultas_pre_natal.mais_6,
        data.consultas_pre_natal.ate_6
    );
    document.getElementById(`status2-${panelId}`).textContent = getStatusIndicator(perc2);
    ...
    document.getElementById(`status3-${panelId}`).textContent = getStatusIndicator(perc3);

    const perc4 = calculatePositivePercentage(
        data.plano_parto.sim,
        data.plano_parto.nao
    );
    document.getElementById(`status4-${panelId}`).textContent = getStatusIndicator(perc4);

    const perc5 = calculatePositivePercentage(
        data.participou_grupos.sim,
        data.participou_grupos.nao
    );
    document.getElementById(`status5-${panelId}`).textContent = getStatusIndicator(perc5);

    // Criar gráficos (mesma lógica da aba geral)
    criarGraficoPizza(`chart1-${panelId}`, ['Sim', 'Não'], [data.inicio_pre_natal_antes_12s.sim, data.inicio_pre_natal_antes_12s.nao], ['#4CAF50', '#f44336']);
    criarGraficoBarra(`chart2-${panelId}`, ['≥ 6 consultas', '< 6 consultas'], [data.consultas_pre_natal.mais_6, data.consultas_pre_natal.ate_6], ['#2196F3', '#FF9800']);
    criarGraficoPizza(`chart3-${panelId}`, ['Completo', 'Incompleto', 'Não avaliado'], [data.vacinas_completas.completa, data.vacinas_completas.incompleta, data.vacinas_completas.nao_avaliado], ['#4CAF50', '#FF9800', '#9E9E9E']);
    criarGraficoBarra(`chart4-${panelId}`, ['Sim', 'Não'], [data.plano_parto.sim, data.plano_parto.nao], ['#2196F3', '#f44336']);
    criarGraficoBarra(`chart5-${panelId}`, ['Participou', 'Não participou'], [data.participou_grupos.sim, data.participou_grupos.nao], ['#4CAF50', '#9E9E9E']);
}
    ...
    fetch('/api/unidades_saude')
        ...
        .then(data => {
            if (data.success && data.unidades) {
                ...
                // Popular select de comparação
                const comparacaoUnidadeSelect = document.getElementById('comparacao-unidade');
                if (comparacaoUnidadeSelect) {
                    unidadesSaude.forEach(unidade => {
                        const option = document.createElement('option');
                        option.value = unidade;
                        option.textContent = `🏥 ${unidade}`;
                        comparacaoUnidadeSelect.appendChild(option);
                    });
                }
            }
        })
...
fetch('/api/indicadores')
    ...
    .then(data => {
        ...
        document.getElementById('status1-geral').textContent = getStatusIndicator(perc1);
        ...
        document.getElementById('status2-geral').textContent = getStatusIndicator(perc2);
        ...
        document.getElementById('status3-geral').textContent = getStatusIndicator(perc3);
        ...
        document.getElementById('status4-geral').textContent = getStatusIndicator(perc4);
        ...
        document.getElementById('status5-geral').textContent = getStatusIndicator(perc5);

        // Usar funções auxiliares para criar gráficos
        criarGraficoPizza('chart1-geral', ['Sim', 'Não'], [data.inicio_pre_natal_antes_12s.sim, data.inicio_pre_natal_antes_12s.nao], ['#4CAF50', '#f44336']);
        criarGraficoBarra('chart2-geral', ['≥ 6 consultas', '< 6 consultas'], [data.consultas_pre_natal.mais_6, data.consultas_pre_natal.ate_6], ['#2196F3', '#FF9800']);
        criarGraficoPizza('chart3-geral', ['Completo', 'Incompleto', 'Não avaliado'], [data.vacinas_completas.completa, data.vacinas_completas.incompleta, data.vacinas_completas.nao_avaliado], ['#4CAF50', '#FF9800', '#9E9E9E']);
        criarGraficoBarra('chart4-geral', ['Sim', 'Não'], [data.plano_parto.sim, data.plano_parto.nao], ['#2196F3', '#f44336']);
        criarGraficoBarra('chart5-geral', ['Participou', 'Não participou'], [data.participou_grupos.sim, data.participou_grupos.nao], ['#4CAF50', '#9E9E9E']);
    })
    .catch(error => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('tab-geral').addEventListener('click', () => trocarAba('geral'));
});